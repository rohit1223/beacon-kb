...
- Stale-RUNNING reaper startup call: Epic 02 sync engine must call SyncJobRepo.fail_stale_running() at process startup and at the beginning of each sync to reap jobs that were RUNNING when the prior process died. The method is implemented in SyncJobRepo but not yet wired to a caller.
- Physical collection name on revision rows: Epic 02 should record the physical Qdrant collection name (e.g. the __rev_<id> shadow name) on revision rows so the engine can look up storage without parsing shadow-name conventions. A migration alongside Epic 02 will add this column.
...
- Sources config_json column: the current sources.config column stores connector config as a comma-separated wire format for list fields (include_globs, exclude_globs). A proper config_json TEXT NULL column should be added in a later migration (Epic 02.5+) to store the full config dict as JSON, making reconstruction in FolderConnector unambiguous and extensible to future connector kinds.
