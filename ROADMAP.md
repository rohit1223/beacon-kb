...
- ParserContract suite - still absent; StoreContract (Task 02.1.1) and ChunkerContract (Task 02.3.1) have landed, ParserContract should follow the same pattern against the Epic 02 parsers.
- Sparse.py per-column bm25() weighting adoption - Epic 02 migration 0002 landed the multi-column chunks_fts schema (text, heading, code); BM25SparseRetriever must now adopt per-column bm25() weights via the store's weighted retrieve(), replacing the current exact-token OR-boost approach (Epic 03, Task 03.1.3 or follow-up).
